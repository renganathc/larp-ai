...
class AnimationsHandler:
    def __init__(self, parent):
        ...
        self.blink_duration = 0.4  # seconds
        ...
        self.idle_acceleration = 0.4  # Acceleration factor
        self.idle_deceleration = 0.9  # Deceleration factor (friction)
        self.idle_max_velocity = 9  # Maximum velocity
    ...
    def update_animations(self, current_time):
        ...
        if self.idle_mode:
            ...
            if not hasattr(self, "idle_target_x"):
                self.idle_target_x = self.parent.eye_l_x
                self.idle_target_y = self.parent.eye_l_y

            # Check if we need a new target
            if (not self.idle_moving and 
                current_time - self.idle_mode_last_time > self.idle_mode_interval + random.uniform(0, self.idle_mode_variation)):

                # Pick a NEW random target (continuous space, not directions)
                range_x = 40   # how far eyes can move horizontally
                range_y = 30   # vertical range

                self.idle_target_x = self.parent.eye_l_x_default + random.uniform(-range_x, range_x)
                self.idle_target_y = self.parent.eye_l_y_default + random.uniform(-range_y, range_y)
            ...
            if self.idle_moving:
                ease = 0.08   # smaller = smoother, larger = faster

                # Move LEFT eye
                self.parent.eye_l_x_next += (self.idle_target_x - self.parent.eye_l_x_next) * ease
                self.parent.eye_l_y_next += (self.idle_target_y - self.parent.eye_l_y_next) * ease

                # Move RIGHT eye (same offset)
                dx = self.parent.eye_l_x_next - self.parent.eye_l_x
                dy = self.parent.eye_l_y_next - self.parent.eye_l_y

                self.parent.eye_r_x_next = self.parent.eye_r_x + dx
                self.parent.eye_r_y_next = self.parent.eye_r_y + dy

                # --- ARRIVAL CHECK ---
                if (abs(self.parent.eye_l_x_next - self.idle_target_x) < 1 and
                    abs(self.parent.eye_l_y_next - self.idle_target_y) < 1):
