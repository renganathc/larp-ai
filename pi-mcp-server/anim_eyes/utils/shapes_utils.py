...
class ShapesHandler:
    ...
    def _draw_sad(self, screen, color, bg_color, x, y, width, height, is_left_eye):
        # Same radius logic
        common_radius = min(width, height) // 3
        common_radius = min(common_radius, width // 2, height // 2)
        common_radius = max(0, common_radius)

        # Same angle logic (but we’ll tone it down slightly)
        angle_height = height // 3   # smaller than angry → softer
        angle_width = width
        angle_height = max(0, angle_height)
        angle_width = max(0, angle_width)
        ...
        # 2. CUT OUTER CORNER (this is the ONLY real change)
        cut_points = []
        if is_left_eye:
            # LEFT eye → outer = LEFT side
            cut_points = [
                (x - 1, y),                      # top-left (outer)
                (x + angle_width, y),            # top-right
                (x - 1, y + angle_height)        # down on outer side
            ]
        else:
            # RIGHT eye → outer = RIGHT side
            cut_points = [
                (x + width - angle_width, y),    # top-left
                (x + width + 1, y),              # top-right (outer)
                (x + width + 1, y + angle_height)
            ]
